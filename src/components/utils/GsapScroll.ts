...
export function setCharTimeline(
  ...
  const tl2 = gsap.timeline({
    scrollTrigger: {
      ...
      start: "center 55%", end: "bottom top",
      scrub: true, invalidateOnRefresh: true,
    },
  });
  ...
  // Screenlight pulse (bindi / cap orb)
  ...
  // Monitor (Plane004) — only if original model
  if (monitor) {
  ...
  // Safe fallback objects for GSAP (avoids crash if null)
  const safeMonitor      = monitor?.children?.find((c: any) => c.material?.name === "Material.027") || { material: { opacity: 0 }, position: new THREE.Vector3() };
  const safeScreenLight  = screenLight || { material: { opacity: 0 } };
  const neckBone         = character?.getObjectByName("spine005") || character?.getObjectByName("neck") || new THREE.Object3D();
  ...
  if (window.innerWidth > 1024) {
    if (character) {
      ...
      tl2
        ...
        .fromTo(".character-model",
          { pointerEvents: "inherit" },
          { pointerEvents: "none", x: "-12%", delay: 2, duration: 5 }, 0)
        .to(character.rotation, { y: 0.92, x: 0.12, delay: 3, duration: 3 }, 0)
        .to(neckBone.rotation,  { x: 0.6,  delay: 2, duration: 3 }, 0)
        .to(safeMonitor.material,     { opacity: 1, duration: 0.8, delay: 3.2 }, 0)
        .to(safeScreenLight.material, { opacity: 1, duration: 0.8, delay: 4.5 }, 0)
        .fromTo(".what-box-in", { display: "none" }, { display: "flex", duration: 0.1, delay: 6 }, 0)
        .fromTo(safeMonitor.position, { y: -10, z: 2 }, { y: 0, z: 0, delay: 1.5, duration: 3 }, 0)
        .fromTo(".character-rim",
          { opacity: 1, scaleX: 1.4 },
          { opacity: 0, scale: 0, y: "-70%", duration: 5, delay: 2 }, 0.3);
...
export function setAllTimeline() {
  ...
  careerTimeline
    ...
    .fromTo(".career-dot",
      ...
      { animationIterationCount: "1", delay: 0.3, duration: 0.1 }, 0);
